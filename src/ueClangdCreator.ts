...
export class UeClangdCreator {
    ...
    protected async setVSCodeSettings(defaultSettings: ValuesOfAllDefaultSettings, workspaceFolder: vscode.ConfigurationScope) {
        ...
        const settings = defaultSettings?.settings as CfgSettings<string, CfgSettingValue> | undefined;  // TODO: Do better?

        
        if(section === undefined || !settings){  
            console.error(`${tr.left_COULDNT_GET_SECTION_section_name} ${section ?? ""} ${tr.right_COULDNT_GET_SECTION_setting_name} ${settings ? "value" : ""}`);
            return;
        }
        ...
        for (const settingKey in settings) {
            if (Object.prototype.hasOwnProperty.call(settings, settingKey)) {
                ...
                if (value !== undefined) {
                    ...
                    if (typeof value === 'object') {
                        if (typeof sectionSetting.value !== 'object') {
                            console.error(tr.TRYING_TO_COMPARE_TWO_DIFF_TYPES);
                            continue;
                        }
                        ...
                        if (typeof value !== typeof sectionSetting.value) {
                            console.error(`${tr.TRYING_TO_COMPARE_TWO_DIFF_TYPES} 2`);
                            continue;
                        }
                ...
                if (value !== undefined && !this.overwrite) {
                    continue;
                }
                ...
                try {
                    ...
                    console.error(`${tr.COULDNT_UPDATE_SETTING} ${section}`);
