...
export const EXTENSION_VERSION = "3.4.1";
...
const ueClangdSettingNames: UeClangdSettingNames = {
    ...
    settings: {
        ...
        "systemIncludes.showMissingWarning": "systemIncludes.showMissingWarning",
        "creation.LinuxTarget": "creation.LinuxTarget"
    }
};
...
export const YAML_OPTIONS_DEFAULT: DocumentOptions & SchemaOptions & ParseOptions & CreateNodeOptions & ToStringOptions
    = {lineWidth: 0, collectionStyle: "block", directives: true};
export const CLANGD_STRINGIFY_OPTIONS = YAML_OPTIONS_DEFAULT;//{directives: true};
export const CLANG_FORMAT_STRINGIFY_OPTIONS = YAML_OPTIONS_DEFAULT;//{directives: true};
export const CLANG_TIDY_STRINGIFY_OPTIONS = YAML_OPTIONS_DEFAULT;//{directives: true};
...
export const completionHelperCppContent = `/*${EOL}` +
    `*  This extension adds completion helper to compile_commands.json and opens it on start.${EOL}` +
    `*  - We use this in compile_commands.json, instead of preparse includes, to speed up code completion${EOL}` +
    `*  - We open this file on startup to add Unreal Macros to code completion (Unreal uses a lot of Macros)${EOL}` +
    `*${EOL}` +
    `*  You need to edit this to include any plugin headers or other important headers missing${EOL}` +
    `*${EOL}` +
    `*  I'll update this based on user recommendations${EOL}` +
    `*/${EOL}` +
    EOL +
    `// Warning: Intellisense use only. Do not include these next three includes in your own code!${EOL}` +
    `#include \"Engine.h\"${EOL}` +
    `#include \"Runtime/Engine/Public/EngineSharedPCH.h\"${EOL}` +
    `#include \"Editor/UnrealEd/Public/UnrealEdSharedPCH.h\"${EOL}` +
    `${EOL}// Kismet Math${EOL}#include \"Kismet/KismetMathLibrary.h\"${EOL}`;

export const completionHelperCppMPcontent = `\n// For Unreal Multiplayer\n#include \"Net/UnrealNetwork.h\"${EOL}`;
